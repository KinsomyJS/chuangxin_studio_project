...
/**
 * Created by FJS0420 on 2016/1/20.
 */
 ...
 * <p/>
 * public interface OnParserCallBack{
 * void onParserComplete(List<HoursWeatherBean> list,PMBean pmBean,WeatherBean weatherBean);
 * }
 * <p/>
 * private WeatherServiceBInder bInder = new WeatherServiceBInder();
 *
 ...
 * <p/>
 * data.getByCitys(city, 2, new JsonCallBack() {
 * @Override public void jsonLoaded(JSONObject jsonObject) {
 * count++;
 ...
 * }
 * <p/>
 * private PMBean parserPM(JSONObject jsonObject){
 * PMBean pmBean = null;
 * try {
 ...
 * if(error_code == 0 && code ==200) {
 * pmBean = new PMBean();
 * JSONObject pmJson = jsonObject.getJSONArray("result").getJSONObject(0).getJSONObject("citynow");
 ...
 * }
 * <p/>
 * private List<HoursWeatherBean> parserForecast3h(JSONObject jsonObject){
 * List<HoursWeatherBean> list = null;
 * SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
 * Date date = new Date(System.currentTimeMillis());
 * <p/>
