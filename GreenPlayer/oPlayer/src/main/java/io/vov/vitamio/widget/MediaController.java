...
public class MediaController extends FrameLayout {
	...
	private void initControllerView(View v) {
		...
		mSeek = (SeekBar) v.findViewById(R.id.mediacontroller_seekbar);
		if (mSeek != null) {
			if (mSeek instanceof SeekBar) {
				SeekBar seeker =  mSeek;
	...
	private long setProgress() {
		...
		if (mSeek != null) {
			if (duration > 0) {
				...
				mSeek.setProgress((int) pos);
			}
			int percent = mPlayer.getBufferPercentage();
			mSeek.setSecondaryProgress(percent * 10);
		}
